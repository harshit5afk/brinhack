...
- **Advanced Document Processing:** 
  ...
  - `Tess4j` for OCR (Optical Character Recognition) on scanned materials
...
1. **User Authentication:** Students and admins log in securely via JWT authentication.
2. **Material Ingestion:** Admins can upload PDFs, DOCX files, or images. The backend uses OCR and text extraction tools to read the content.
3. **AI Generation:** The extracted text is processed by Ollama via Spring AI to dynamically generate mock test questions.
4. **Taking the Test:** Students attempt tests on the React frontend or Flutter app. During the test, `face-api.js` monitors the student for proctoring.
5. **Analytics & Progress:** Once the test is completed, scores are stored in MySQL, and the frontend displays personalized analytics and progress charts.
6. **Smart Reminders:** A background cron job checks for student inactivity. If a student hasn't taken a test in 2 days, Twilio triggers an automated friendly IVR voice call to encourage them to practice.
...
mvn spring-boot:run
```
*(Make sure to update your `application.properties` with your database credentials and Twilio tokens. Note: `application.properties` is intentionally ignored in version control for security).*
